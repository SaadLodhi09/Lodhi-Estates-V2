...
export function ContactForm({ property }: ContactFormProps) {
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const parsed = inquirySchema.safeParse({
      ...
      website: data.get('website'), // honeypot — real users never see this field
    });
    ...
    // Honeypot tripped: silently no-op rather than showing an error, so a
    // bot filling every field learns nothing from the response.
    ...
    return (
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-ink/65">
          ...
          {user && ' You can also track this in your account under Inquiry History.'}
        </p>
      </motion.div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8" noValidate>
      ...
      {/* Honeypot field — hidden from real users via CSS and tabIndex, but visible to naive bots. */}
