...
export default function AdminLogin() {
  ...
  if (!loading && isAuthenticated) {
    // ProtectedRoute (wrapping /admin) will bounce a non-admin session
    // straight back to "/" — this redirect just gets them there.
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // session and the redirect above will fire on next render.
    } catch (err) {
      setStatus('error');
    ...
    <div className="flex min-h-screen items-center justify-center bg-paper px-6">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-8 text-xs leading-relaxed text-stone">
          This is for site admins only. Buying or selling a property?{' '}
