...
There's no separate admin sign-up — you create a normal account, then
promote it:

1. Once the site is running (see step 4 below to get that far), go to
   `/account/sign-up` and create an account with your own email —
   exactly like any client would.
2. Back in Supabase Studio → **SQL Editor**, run (with your real email):
   ```sql
   update public.profiles set role = 'admin' where email = 'you@example.com';
   ```
3. Sign in at `/admin/login` with that same email/password — you're now
   an admin. Repeat step 2 for any other admin accounts you want.
